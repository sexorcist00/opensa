...
function bootstrap(
  ...
  bootstrapped ??= (async (): Promise<Bootstrap> => {
    ...
    const game = Game.getInstance(canvas, createGameRuntimeConfig());
    ...
    if (new URLSearchParams(window.location.search).get('webgpu') === '1') {
      // Bundle bug hunt (docs/plans/073-webgpu-migration-threejs/concept): `?bundledebug=1` turns on the patched three's
