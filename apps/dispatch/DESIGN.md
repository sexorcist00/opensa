...
| [Tickets CAD](https://openises.sourceforge.net/) (Open ISES)                                                          | the volunteer-agency open-source CAD since 2005; v4.0 "NewUI" in development                    | **No screen could be captured 2026-08-28** (Cloudflare challenges SourceForge from this egress, `ticketscad.org` fails TLS, `web.archive.org` unreachable), so it is a row and not a preset — a skin built from the words "modern dark theme" is our own taste under a borrowed name. What its own page states is still worth two things: **light and dark as an instant toggle, both "designed for extended use"** — a better justification for Day than the one written here — and a drag-and-drop widget dashboard, which is Resgrid's BigBoard for the third time and gets 7-08's answer for the third time |
