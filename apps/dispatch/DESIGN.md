...
| [Tickets CAD](https://openises.sourceforge.net/) (Open ISES)                                                          | the volunteer-agency open-source CAD since 2005; v4.0 "NewUI" in development                    | **RUN, not just read — 2026-08-29.** The vendor's own screens are still uncapturable (Cloudflare on SourceForge, TLS failure on `ticketscad.org`), so the **2.41 dev-repo build was stood up locally** (MariaDB + PHP 8 with an `ext/mysql` shim) and measured. It is the CLASSIC line, not the v4 NewUI. What it settles: **the skin is chosen at LOGIN** (a `Colors: Day / Night` radio on the sign-in form, held per session) and it is a pure palette swap — same layout, same controls, same positions, which is [7-09](../../docs/plans/201-dispatch-console/7-the-operator-map/readme.md)'s rule arrived at independently. What it warns: the swap is **partial and unmeasured** — in Night, the Call Board's `Show` link is black `#000` on the `#121212` ground (**Lc 0, invisible**), unit rows are 10 px black on `#9e9e9e` (Lc 52) and the column-group headers white on `#99b2cc` (Lc 48), against our Lc 90 / 60. Rows are 14 px tall with 10 px data text — a desk density with no phone answer. And **status colour is agency-editable DATA** (`un_status.bg_color` / `text_color`, `in_types.color`) — the opposite of our rule, and the reason ours cannot follow: our chips have to equal what the engine draws |
