...
fn vsWorld(in: VsIn) -> VsOut {
  ...
  out.sunNdl = max(dot(normalize(in.normal.xyz), frame.sunDir.xyz), 0.0) * sunVis;
...
fn fsWorld(in: VsOut) -> @location(0) vec4f {
  ...
  let lit = in.prelit * (frame.params.y * in.ao) + frame.sunColor.rgb * (in.sunNdl * frame.params.z);
