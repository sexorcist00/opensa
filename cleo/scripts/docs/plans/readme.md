...
| 3   | [003 — cutscene viewer](003-cutscene-viewer.md) | a real-SA debug instrument: cheat code → cycle the vehicle cutscenes with `[`/`]`; scene list generated offline by vehicle-cutscene's tooling (census + the ANPK channel reader). **Single-target by design** (OpenSA has no cutscene system) — the dual-target bar does not apply | PLANNED 2026-08-12 — next up |
