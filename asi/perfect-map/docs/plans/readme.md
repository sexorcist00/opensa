...
- The full perfect-map build survives by staying under **30,000 text rows** — `checkTextIplBudgets` on the built `sa/` tree ([docs/open-issues/fixed/ghost-barriers.md](../../../../docs/open-issues/fixed/ghost-barriers.md)). That caps how much map/vegetation/procobj we can ship, and it is now the ONLY such cap: the 39-slot ceiling became a report on 2026-08-08 (the target's OLA lifts that array), so int16 — this asi's patch #1 — is the whole remaining story.
...
| 6   | [006 — Pipeline integration & budget lift](006-pipeline-integration.md)                                                                       | ship the asi from pmb output; relax `checkTextIplBudgets`' int16 gate when it's present (its slot half and its "stock mode" premise are already gone — read the banner)                                            | idea    |
...
- **In-repo**: `tools/perfect-map-builder/src/pipeline.ts` (`checkTextIplBudgets`, on the `sa/` branch), `tools/map-placement/*` (binary streams), [ghost-barriers.md](../../../../docs/open-issues/fixed/ghost-barriers.md) (the full post-mortem this chain acts on).
