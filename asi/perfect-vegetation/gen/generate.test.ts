...
describe('perfect-vegetation catalogue', () => {
  describe('positive cases', () => {
    ...
    it('renders the catalogue with one anchor per site', () => {
      const hpp = renderHeader(CATALOGUE, SA_FINGERPRINT, { namespace: 'pveg' });
      ...
      expect(hpp).toContain('namespace pveg::gen {');
    });
    ...
    // (No `length > 0` assertion yet: the catalogue is empty until plan 001 step 1 — add it with the sites.)
