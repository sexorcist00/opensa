...
| dispatch: a PHONE pak    | `npx tsx tools/opensa-pack/src/cli.ts --game ./game-src/original --out ./build/district --rgba8 --rect 4,4,11,11` → serve it and open `dispatch.html?src=build/district`. `--rgba8` is what makes the pak loadable on a GPU without BC (every mobile one); `--rect` keeps the 4-8x texture cost affordable. Cells are `floor(worldXY / 250)`, so `4,4,11,11` is GTA x/y 1000…3000 |
