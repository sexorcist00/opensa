...
| Clutter A/B              | `&procobj=<×density, 0=off, saturates at 3>` · `&procobjLimit=<per-cell, saturates at 300>` · `&procobjRange=<units, overrides every category's draw distance — 150 = the pre-2026-08-10 ring, 100 = SA's own flat PLANTS_MAX_DISTANCE>` · `&procobjFloor=<n, 0=off — keep at least n of every species the cell is eligible for; 1 removes the 17.7 % species loss for 0.32 % of the drawn placements>`. Full table: [query-parameters.md](development/query-parameters.md) |
