...
| the phone panel as MCP    | `npm run panel:tunnel` — the MCP server plus a cloudflared tunnel in one command, printing the two values to paste (`OPENSA_PHONE_URL`, `OPENSA_PHONE_TOKEN`) and keeping the token in `build/.phone/mcp-token` so a restart re-pastes one value, not two. `npm run panel:mcp` is the stdio transport, for a Claude running ON the phone. Either way it exposes the panel's own capabilities as tools — state/preflight, the job allowlist, run, log, stop, commit — so an agent drives a field run without a person relaying the screen. A CLIENT of the running panel (one job runner per phone); `phone_exec` is a real shell and is OFF unless `PANEL_MCP_EXEC=1`. [plan](../tools-debug/phone-console/docs/plans/002-mcp.md) |
