...
| the phone panel           | `npm run panel` → `http://localhost:8787` on the phone, then add it to the home screen. Preflight (why a run will not start), the rituals as buttons, a live log, and the capture inbox that writes `docs/benchmarks/opensa-engine/<date>-<name>.json` and commits it. Binds `127.0.0.1`; `PANEL_HOST=0.0.0.0` opts into the LAN and says so. [tools-debug/phone-console](../tools-debug/phone-console/README.md) |
