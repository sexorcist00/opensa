...
| Clutter A/B              | `&procobj=<×density, 0=off, saturates at 3>` · `&procobjLimit=<per-cell, saturates at 300>` · `&procobjRange=<units, overrides every category's draw distance — 150 = the pre-2026-08-10 ring, 100 = SA's own flat PLANTS_MAX_DISTANCE>` · `&procobjFloor=<n, DEFAULT 1 since 2026-08-11 — keep at least n of every species the cell is eligible for; 0 is the A/B and brings back the 17.7 % species loss>`. Full table: [query-parameters.md](development/query-parameters.md) |
