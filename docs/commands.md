...
| a killed convert          | `npm run phone` again — it resumes from `<out>/.pack-checkpoints` (the pack journals every weld chunk). `REBUILD=1` deletes the journal and starts over. Android kills Termux on this device even with the screen on: [termux.md](development/termux.md) has the EMUI settings and the Android 12+ phantom-process limit |
