...
| Tier policy / F2 / CI coverage (plan 07) | PHASE A DONE 2026-08-05 (`tiers.ts` declared gaps + runner `coverage()` + corpus-coverage CI join, 112 tests); tracer, F2 screen, README, trace fixtures, audit+benchmark close-out remain |
| Field bug round (2026-08-05, in the 07 ledger) | FIXED+VERIFIED headless: world-array eviction crash at the wheel (`Engine.releaseWorldArray`), coach/bus collapsed `wheel_lf` (orphan vertex — `wheelRadius` reads referenced vertices), coach boarding (engine `doorSides` reads the model's door parts — the Car Left Door script stays class-C-inert), footprint-based `ENTER_RANGE`, size-aware F2 spawn. Wheel BLINK deferred → [plan 099](../plans/099-script-object-uv-anim/readme.md); rhino tracks stay on the rig block (user: superseded by 097/08 authoring) |
