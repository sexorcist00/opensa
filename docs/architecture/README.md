...
packages/                          (tag type:engine)
  ...
  audio/          @opensa/audio           the world's own sound (plan 203): the context and its autoplay
                                          lifecycle, the voices and listener above it, three mixer buses
                                          (world/map/cad, one owner each), and the per-zone ambience bed.
                                          ZERO runtime dependencies — the console ships as an embeddable
                                          widget
