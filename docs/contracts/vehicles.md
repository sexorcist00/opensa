...
| window-glass materials (classified by DATA, no names: translucent below alpha 200, off the `vehiclelights*` atlas, dark-tinted or alpha ≤ 128) | Alpha CLAMPED to the vanilla twin's most transparent window pane (plan 004, BCESAR5 round). The cutscene path draws a clump's translucent panes unsorted, so stacked windows compound — R* authored each cs model's window alpha to its scenes' stacking depth (zr350 26, boxy cars 77–128, fleet ceiling 128) while mod glass ships its gameplay tint (~101–193), which turns into an opaque wall two panes deep. Slots whose vanilla twin has no glass modelled fall back to the fleet median 102. Lenses/decals (bright, alpha 150+) and lamp-atlas materials are untouched. A mod's window tint is therefore LIGHTENED in cutscenes by design; its gameplay look is unchanged. |
...
| SA Pipeline Set plugin (`0x253f2f3` = `0x53F2009A`, vehicle pipeline) | Stamped into EVERY emitted atomic's Extension when missing (plan 004 round 6) — every vanilla cs atomic carries it. A gameplay DFF gets the vehicle pipeline from its model-info type, so mods never ship the plugin; a cutscene object without it renders on the default pipeline where the mod's Reflection/Specular material plugins go unread (a different shine than the same car in gameplay). |
