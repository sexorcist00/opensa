...
| [Automatic render-scale / quality-tier ladder](deferred-optimizations/render-scale-tier.md) | GPU pass | 0.4–1.4 ms (measured ceiling, **on an immediate-mode GPU**), targets 345 → 88 MB | policy refused; the LADDER is re-run on a tiler by [201/9-04](../plans/201-dispatch-console/9-the-mobile-frame/readme.md) |
| [MSAA sample count as a budget number](deferred-optimizations/msaa-sample-count.md) | memory (render targets) | ~22 MB on a phone — 65.7 % of the `target` category, 29 % of the whole 74.9 MB residency measured 2026-08-12 | **REOPENED 2026-08-31 by [201/9-04](../plans/201-dispatch-console/9-the-mobile-frame/readme.md)** — the file priced MEMORY only; `target` is now 62 % of the phone's residency AND the 48 B/px tile working set is a frame-time question at 21 fps |
...
| [A unit's `_vlo` mesh at city zoom](deferred-optimizations/unit-model-lod-band.md) | triangles (the fleet) | unmeasured — every converted car already ships the low-detail mesh in the same buffers, and the console draws the full body at every zoom | in reserve — 201/5-04's own number (150 units with models, on a phone) is unpaid, and switching first would spend the honest worst case the first capture is for |
| [The glass is still one draw per car per submesh](deferred-optimizations/vehicle-blend-phase-instancing.md) | draw calls | 201/9-08 instanced the OPAQUE vehicle phase (11 810 draws → 3 571, triangles identical) and left the BLEND phase alone: its submesh order is a function of the eye and differs per car, so one draw cannot serve two of them. Most of the remaining ~3 459 car draws are that phase | in reserve — the whole fleet measures **+0.9 ms** of a 23.8 ms board, the frame's largest term is the post chain on a 20.2 ms empty map, and a mis-grouped blend run renders plausibly wrong where a mis-grouped opaque run is caught by a test |
...
| [Env-probe cadence and resolution](applied/env-probe-cadence.md) | GPU pass | **1.6 ms of a 21.5 ms frame** measured by ablation on the 2/03 phone (201/9) | **PULLED 2026-09-05 — the quality-free half**: the refresh is gated on a reflective instance existing. Cadence, face size and resolution untouched and still in reserve |
...
| [Automatic render-scale / quality-tier ladder](deferred-optimizations/render-scale-tier.md) | **low** frame · **medium** memory | GPU pass | 0.4–1.4 ms (measured ceiling, **on an immediate-mode GPU**), targets 345 → 88 MB | policy refused; the LADDER is re-run on a tiler by [201/9-04](../plans/201-dispatch-console/9-the-mobile-frame/readme.md) |
...
| [Env-probe cadence and resolution](applied/env-probe-cadence.md) | **low**, **medium** when hot | GPU pass | **1.6 ms of a 21.5 ms frame** measured by ablation on the 2/03 phone (201/9) | **PULLED 2026-09-05 — the quality-free half**: gated on a reflective instance existing; cadence and face size still in reserve |
