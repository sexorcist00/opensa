...
| [Dedupe the clone-LOD textures into a `txdp` parent again](deferred-optimizations/salod-txdp-parent-dedup.md) | **none per frame** | archive bytes | 10.4 MB against the 45.9 MiB the self-contained dictionaries measure today | **GIVEN UP for correctness 2026-08-16** — the real game does not deliver a `txdp` parent's textures to the child's materials: every parent-only name rendered UNTEXTURED, and 49 % of 4 050 clone LODs depended on one. Comes back only on a field-proven parent, one texture, photographed |
