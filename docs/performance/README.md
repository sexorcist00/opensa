...
| [Turn the clutter species-roster floor back off](deferred-optimizations/procobj-species-roster-floor.md) | `sa` object count · `opensa` per-cell CPU | 312 permanent rows of 91 379 (0.34 %) on `sa`; **nothing measurable** on `opensa` (the floor swaps a placement, never adds one) | cost TAKEN 2026-08-11 — very low effort to give back (two config values), and 17.7 % of clutter cells lose a species if you do |
