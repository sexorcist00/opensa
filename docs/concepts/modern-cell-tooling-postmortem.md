...
| Per-model atlas (ignoring tiling) | −78 % *(mirage)* | not achievable — tiling |
...
- The **baked channel idea** (AO/emissive per vertex) may feed the WebGPU material path — e.g. a baked-AO channel
  in the format could replace the screen-space SSAO pass (see webgpu-migration/03). If we return here, that's why.
