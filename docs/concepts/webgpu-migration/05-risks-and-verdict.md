...
The entire upside rests on three `0.177`'s render-bundle support collapsing per-frame submission for a *streaming*
...
→ **Mitigation:** port it *first* (Phase 1), gate on the map-viewer compare workflow across a dawn→night sweep.
...
the realistic first result is ~30 fps, not 60. Going higher is *more* work (triangles, post-FX cost) — WebGPU
...
- **Triangle count / GPU time** — orthogonal; that's LOD/decimation/post-FX work, valuable *after* CPU-unbound.
