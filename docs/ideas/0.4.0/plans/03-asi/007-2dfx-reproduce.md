...
Part of the [opensa-asi chain](readme.md), Phase 2 — **the very first task of Phase 2, before any RE or patching.** The 2dfx emitter fix ([008 RE](008-2dfx-emitter-re.md) → [009 patch](009-2dfx-emitter-patch.md) → [010 pipeline](010-pipeline-keep-2dfx.md)) needs a fast, deterministic reproduction of the leak/crash to use as its pass/fail oracle — the Phase-2 analogue of [000](000-reproduce-bug.md). You cannot confirm a fix you can't reliably trigger.

## Context

The open issue ([lod-2dfx-particles.md](../../../../../docs/open-issues/lod-2dfx-particles.md)) reproduced this once, via the full perfect-map pipeline with particle 2dfx left on cloned LODs: cloned-LOD effect emitters never unload → effect/model pool exhaustion → **new-game crash at `0x004AA3A1`** (AV `[null+0x1B]` in `CFileLoader::LoadObjectInstance`, a null model-info on first world-load). Scale: 38 models carry particles, sa-lod cloned 11 of them (refinery smoke, Vegas plants, fountains, fire). We currently STRIP particles at build time so it never fires — the repro must deliberately UN-strip.
...
3. **Isolate from Phase-1's limit bugs.** Keep the repro build within the int16/slot/scene bounds ([000](000-reproduce-bug.md)'s isolation guards) so the crash is attributable to the EFFECT pool, not a text-IPL/pool ceiling. `0x004AA3A1` (LoadObjectInstance null model-info) vs Phase-1's `0x404C90`/`LoadIplBoundingBox` addresses already distinguish them — assert the crash address to prove which bug fired.
...
- [ ] Repro build: a sa-lod flag that keeps particle 2dfx on the clones (all 11, or a dialled subset); produce an asset set that crashes at `0x004AA3A1` on new game in stock 1.0 (Wine). Confirm the crash address.
...
- min emitter count to crash / time-to-crash vs count: …
- crash address confirmed (`0x004AA3A1`) + isolation from Phase-1 addresses: …
- repro command + Wine procedure + leak/crash detection oracle: …
- fx-pool occupancy trace across a new-game load (once instrumented): …
