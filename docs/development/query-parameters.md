...
| `at=x,y` · `h=` · `pitch=` · `yaw=` | opening camera pose (GTA ground point, height, degrees). **These names, `proj`, `hour`, `district`, `src` and `back` are one table read by one module** (`apps/dispatch/src/map/view-link.ts`, 201/7-07) — the console's `Copy link` writes exactly them, so a generated link and a hand-written one are the same thing |
| **`embed=1`** | **201/7-07**: the embedded shell — the map and its own controls, without the queue, roster, timeline or status bar. What an embedded console may and may not do is stated in [features/dispatch-console.md](../features/dispatch-console.md) |
| **`back=`** | **201/7-07**: seconds BEHIND live to open the shift clock at, so a shared link restores the moment as well as the view. Absent or `0` opens live |
...
| **`mode=flat`** | **201/6-02**: open the FLAT 2D map — no WebGPU, no pak, no streaming. The tiles come from one `tiles.pmtiles` beside the built game (`?src=`), read by HTTP range requests; with no archive the mode keeps its projected grid and the status bar carries the reason. The tile layer is exact under the plan projection and under no other, so opening a pyramid takes the view there |
| **`tiles=`** | **201/6-02**: an archive anywhere, overriding `<src>/tiles.pmtiles` |
| **`bake=tiles`** | **201/6-02**: bake the pyramid from the streamed world with the console's own orthographic pass and download `tiles.pmtiles`. `zmin=`/`zmax=` (default `0`/`4`) and `tilesize=` (default 256) bound the run; past 4096 tiles it is refused with the count. It runs in the browser because the development machine has no headless Chromium, and it pauses the console's frame loop while it works. Progress and the closing report are `[tilebake]` console lines |
...
| **`graphics=`** | **201/9-05**: the graphics rung the console opens on — `full`, `balanced` or `smooth` (the default since the operator's night verdict of 2026-09-05). Overrides the operator's stored choice so a shared link reproduces what its sender was looking at; an unknown value is ignored and the stored choice stands. `smooth` is the one with a number on it: **44.6 % of consecutive frame pairs stutter → 7.9 %** with the bloom chain gone ([the row](../benchmarks/opensa-engine/2026-09-05-mobile-frame-pacing.json)). **Not an ablation arm** — `?ablate=bloom` removes the passes for a MEASUREMENT and is fixed for the engine's life; this is the shipping path and takes the intensity to zero, which skips the same passes and can be turned back on. |
| **`surface=WxH`** | **201/9-01**: pin the SCENE's drawing buffer to a fixed pixel count, so arms taken minutes apart are comparable while the browser's chrome collapses and returns. Both edges are clamped to `2..4096` and anything unparseable is **ignored entirely** — a refused value falls back to the viewport, never to a different pinned size, because a capture that silently ran at a size nobody asked for is the failure the module exists to prevent. **It reaches the scene alone**: the 2D overlay is drawn in the DISPLAYED box's coordinates, so a pin costs the symbology nothing and `overlay-2d` is a cost of the CSS box rather than of the pin ([the aspect restriction](../restrictions/architecture.md)). The report's `surface` block states what was pinned |
| **`ablate=`** | **201/9**: remove one group of passes from the frame and re-fly the same route — **the chain's whole instrument**, because there is no `timestamp-query` on the 2/03 device and no browser flag brings it ([edge-cases](../edge-cases/browser-runtime.md), re-tested 2026-09-04), so a pass is priced by its ABSENCE. Names: `cells` (every resident cell's opaque and blend bundles — the cull still runs, so `draws` and `triangles` still report what WOULD have drawn), `cloud` (the 256² two-fbm bake; the world still SAMPLES the texture, so this prices producing it), `bloom` (the whole chain), `probe`, `skylut`. Comma-separated, so `?ablate=bloom,cloud` is one arm removing both; **an unknown name is ignored and the rest of the list still applies**, and the report's `surface.ablated` says what actually ran — an ablated run is otherwise indistinguishable from a fast one. **Read the mean, not p50**, which saturates on the 16.7 ms vsync floor. **AND THE INSTRUMENT'S FLOOR IS ~2.5 ms** ([the null arm](../benchmarks/opensa-engine/2026-09-05-mobile-ablation-null-arm.json)): an arm under it is a sample, not a measurement. **A measurement knob and never a shipping path** — the standing call (2026-09-04) is that frame time may not be bought with resolution, sampling or anti-aliasing |
| **`msaa=` · `scene=`** | **201/9-04**: the scene pass's sample count (`1` or `4` — WebGPU allows no other, and A2C needs 4, so `msaa=1` is a LOOK change that loses alpha-to-coverage) and its colour format (`rgba16float`, `rgb10a2unorm` or `rg11b10ufloat`) — the attachment ladder, 48 bytes per pixel at the default down to 12. **`scene` is a name this repo now uses twice**: the sequence-start parameter in the table above belongs to another page, and nothing catches the collision because the two are different hosts and no test compares them. **Both are EFFECTIVE rather than asked** — the device answers in `resolveRenderBudget` and a format the adapter cannot render falls back — so read `surface` in the report rather than the URL |
| **`bloomlevels=` · `bloomminpx=`** | **201/9-05**: how far the bloom chain is built. `bloomlevels=` pins the level count (integer `2..12`; out of range is ignored and the size derives it, and the engine clamps to 2 besides — the composite binds `upViews[0]` and there are `levels - 1` up views, so a one-level chain binds nothing), `bloomminpx=` is the pixel floor under a level worth building. **Neither is a frame-time lever and neither is sold as one**: the sweep priced `?bloomlevels=4` — dropping the mips that are 11×10, 6×5 and 3×3 px at 720×640 — at **0.2 ms of a 23.4 ms frame, which is noise**. The tile-flush argument the step opened on is not what this device does; the money is in the FIRST levels. What the floor buys is six fewer textures, bind groups and uniforms for mips smaller than a chip |
| **`bloomformat=` · `bloomscale=`** | **201/9-05's two REAL levers**, after the sweep refuted the level count. `bloomformat=` gives the chain's own targets a format apart from the scene's — `rg11b10ufloat` halves the bytes of every pass that reads or writes the chain and measured **−2.4 ms**. `bloomscale=` is where the pyramid STARTS as a fraction of the render size (`0.5` or `1`): half quarters the three passes that are ~90 % of the chain and measured **−4.4 ms**. **They are ALTERNATIVES, not additive** — together they read the same as half alone ([the levers](../benchmarks/opensa-engine/2026-09-05-mobile-bloom-levers.json)). **`bloomscale=0.5` SHIPPED as this console's default** on 2026-09-05 and is a LOOK change (the bright-pass threshold then runs on a 2×2 average, so a sub-pixel emitter can be diluted below it) — it released a protected-list item, and only by the operator's verdict on the device, taken at NIGHT because a daylight A/B on it settles nothing. `?bloomscale=1` re-flies the old baseline, since a default that moved without leaving its predecessor reachable would make every earlier row unrepeatable |
| **`bloomdown=` · `postprec=`** | **201/9-05b, the vendor levers** (Bjørge/Arm, [links](../links.md)): the downsample's kernel — `dual5`, dual filtering's five taps instead of `box13`'s thirteen — and the precision the bloom and post COLOUR maths run at, `f16` against `f32`, which Arm prices at roughly 2× on their ALUs. **Both provably do less work and the frame does not notice** ([the row](../benchmarks/opensa-engine/2026-09-05-mobile-vendor-levers.json)), and that prediction was pre-registered: ~90 % of frames already sit on one display interval, and a lever worth tenths cannot be seen from under a vsync floor. Keep them for a device that is not this one |
| **`models=0`** | **201/9, §6**: the FLEET's control arm — the board keeps its 150 units and none of them is drawn as a car. It hands the model layer no source at all, so the model reads, the texture uploads and the vehicle draws all go, and `board` − `models=0` is what the fleet costs in the frame AND in residency. The subtraction had been taken before only across two windows whose difference was an ACCIDENT (a convert without the board's names, a pak served without its game dir), which nobody could re-fly. **The units keep their symbols**: a unit with no car earns its mark back, which is the clause that makes this an arm rather than an empty map — it was missing until 2026-09-08 and every patrolling unit in the shareable demo was invisible without it. The capture's `models` field carries the arm |
| **`audio=0`** | **203/6-01, the SILENT BASELINE**: remove the console's audio entirely — no context, no voice pool, no tick. It REMOVES rather than mutes, the way `?models=0` removes the fleet: a run that still built a context and a pool would measure a silent console rather than a console with no audio, and 4/03 owes a battery figure that can only be read as the difference between the two. The report says which it was (`audio.arm`), and unrecognised is the DEFAULT — a typo may not silently measure a cut-down run |
| **`cad=0`** | **204/3-02**: remove the CAD STAND-IN — the fake that raises `cad` events (a panic, an assistance request, a plate read) so the bus can be heard and measured before PCAD ships the contract's `sound` field. It removes the stand-in ONLY: the seam it speaks through ([`cad-link.ts`](../../apps/dispatch/src/world/cad-link.ts)) is unchanged, which is exactly what the day a real CAD connects looks like. **It is independent of `?audio=0`**, and that is the point: the silent baseline is a console that still RECEIVES its events and only does not voice them. The capture's top-level `cad` says which it was (`online` is `true` when something has spoken, `null` when nothing ever has), `cad.assumed` counts the messages that named no sound and were played as `notification` — a CAD on the old wiring, where everything still chimes but every event chimes the SAME — and `cad.failed` counts sinks that threw, because a dead `AudioContext` may not take the on-screen notice with it |
| **`box=WxH`** | **201/9, §6**: pin the CSS BOX, in CSS pixels — the thing `surface=` deliberately does not touch. The overlay is sized from the box and the browser's chrome collapses and returns mid-flight (360x320, 360x570 and 360x609 inside one session), so two arms taken at two boxes have different overlay pixel counts and `overlay-2d` is the largest CPU line in the frame. Both canvases keep sizing their buffers from the box, so the symbology is still drawn in the coordinates it is displayed in and the pin costs it nothing. Unparseable is ignored entirely, like `surface=`. **The report says it either way**: `surface.cssWidthMin/Max` and `cssHeightMin/Max` are the extremes the window actually reached, from the resize observer rather than a per-frame poll, and a window that mixed two boxes carries a warning naming both. **VERIFIED ON THE DEVICE 2026-09-09** ([the row](../benchmarks/opensa-engine/2026-09-09-mobile-box-pin-verification.json)): the pin holds (`device.css` 360x320, `boxPinned: true`, min == max on both axes), and what it varies is the overlay canvas plus the COMPOSITOR's scale — a 720x640 buffer is a 1:1 blit into a 360x320 box at DPR 2 and a 2.04x upscale into 360x654. **It is not a null arm**: this phone's natural box is 360x654, and the record it was supposed to reproduce carries eleven different boxes. On an empty board the CPU cost of the larger box is not separable from thermal drift |
| **`sprites=0`** | **201/9-08**: rebuild every symbology mark as a path per instance instead of blitting it from the sprite sheet — the symbology's own drawing path, as a control arm. The desk control puts a blit at **~0.275 ms** against a path's **~0.32** ([the row](../benchmarks/opensa-engine/2026-09-05-desk-canvas-symbol-arms.json)), and six alternating windows on the device could not tell them apart |
| **`tick=<ms>`** | **201/9-02**: the mock board's publish interval. The default is **4000** — PCAD's own rate, read out of `cadui.lua` rather than chosen ([202 §4](../plans/202-pcad-dispatch/readme.md)) — and it was **50** until 2026-08-31, which is 80× the interface it stands in for. That mattered because `RenderGate` compares the board by IDENTITY and `stepOperations` returns a fresh object every tick: at 20 Hz render-on-demand could never rest longer than 50 ms, so every drawn-frame rate, `framesSkipped` and battery figure this repo has filed was measured on a console the mock kept awake. `?tick=50` puts the old churn back for a comparison against one of those rows; anything outside 16–60 000 ms falls back to the feed's rate rather than measuring something nobody asked for. **The shift clock is not on this tick while replaying** — playback runs at its own 50 ms so a scrub is not a slideshow, and the board underneath still steps at the feed's rate |
| **`overlay=0`** | **201/2**: draw the WORLD and nothing over it — the symbol pass, the chips, the sketches and the radar stop, and so does the per-frame update that feeds them. For measuring the ENGINE: the field round of 2026-08-30 put `overlay-2d` at 2.37 ms of CPU against `engine-frame`'s 0.57 at 150 units, so a streaming or texture number taken with the overlay on is mostly the symbology's. Subtracting the segment afterwards would not do — the GPU still did the work — so the pass is skipped instead. **The render gate stops watching the board with it**, or a window meant to price the engine would be paced by a roster nobody can see. The layers are still CONSTRUCTED, so this is the draw cost and never the allocation, and the capture carries `overlay: false` so a reader cannot mistake which run they have. `?embed=1&overlay=0` is the engine with nothing above it at all. **`overlay=clear` is the third arm** (201/9-01): the overlay canvas is cleared every frame and nothing is drawn into it, so the compositor pays for the LAYER exactly as it does on the full arm. `off` skips the `clearRect` too, which is why the two-arm A/B could not say whether the 21 unattributed ms was the layer or its content — `clear` minus `off` is the layer, `on` minus `clear` is the content. The capture's `overlay` field carries the arm's name rather than a boolean for the same reason; a row filed before 2026-08-31 carries `true`/`false` |
| **`inventory=1`** | **201/1-01**: collect the frame before-table and show a panel with a copy button. Its `cpu.worstFrame` carries the CONDITIONS that frame happened under — when in the window, the drawing-buffer size (a phone resizes itself when the browser's chrome collapses, and that rebuilds every render target), the cells created and evicted in that frame alone, what was still pending, and the draws — because the 2026-08-23 field capture had a 600 ms frame and nothing to attribute it to |
...
**And the conditions the device was in, since 2026-09-08** — the three §6 lines that had never been recorded,
so every thermal argument in [201](../plans/201-dispatch-console/readme.md) was made without a number:

| Field | Reads as |
| --- | --- |
| `power.charging` · `power.chargingChanged` | which side of the CABLE the row was taken on, and whether it crossed mid-window. A phone on a charger has a different thermal envelope and a different governor, so two arms on opposite sides are not an A/B — and a window the cable moved inside carries no thermal claim at all |
| `power.levelStart` · `power.levelEnd` | the battery over the window. Chrome quantizes `level` to whole percent, so a 60-second delta is usually 0 — this is for the LONG session (4/02) and for the battery figure 4/01's render-on-demand claim owes |
| `power.supported` | `false` where the browser has no Battery Status API (Firefox, Safari, most desks) — an absent reading, never a fabricated full battery |
| `visibility.hiddenMs` · `hiddenSpells` | time this page spent OUT of the foreground. Android suspends a backgrounded tab, and the 2026-08-31 run lost its only flight with the world resident to exactly that; a window with either non-zero was not measured on a page anybody was looking at, and nothing else in the file would say so |
| `audio.panel` · `cad` | **what the PANEL said (204)**: events played per category, folds, the worst **event → audible** gap against the 50 ms budget, and names no build knows. `cad` is top-level rather than under `audio` on purpose — these events reach the SCREEN on a surface that can make no sound, so a capture filing them under the speaker would say a muted console received nothing. The latency is measured from when the event HAPPENED rather than from the call, because a message that took 200 ms to cross a socket and 1 ms to reach a speaker is a late alert |
| `audio.arm` · `availability` · `clock` · `voices` · `absence` | **what could be HEARD (203/6-01)**: the arm, what the browser allows (`waiting` until a touch, `running` after one, `unsupported` where there is no Web Audio), the audio tick's own mean and worst against its 2 ms budget beside the rate it ran at, the voice pool's live/peak/steals — plus, since 204/5-01, **`voices.peakSample`**, the loudest sample that actually LEFT the graph (tapped after the limiter, read on the audio clock; the limiter's own `reduction` is about what went IN, and 1/02's -6 dBFS threshold was chosen on arithmetic nobody had played), and **`voices.peakByBus`**, because *sixty-four voices at once* is an ordinary city and the question a reserve is judged on is how many of them were ALERTS — and every absence — counted exactly, the first 32 named. **A silent world and a broken one look identical from outside**, which is what these fields exist to separate |
| `surface.cssWidthMin/Max` · `cssHeightMin/Max` | the extremes the CSS BOX reached during the window. Max ≠ min means the overlay's pixel count moved inside the row, and the report warns naming both boxes |

**The die TEMPERATURE is not among them and cannot be** — no browser exposes it. The phone panel stamps it
onto the capture's note from `termux-battery-status`, and a device without Termux:API gets a named skip in
the note rather than a silence. **The number of competing browser TABS cannot be measured at all**: no web
API names another tab, and Termux on an unrooted phone cannot see another app's processes — `visibility` is
the honest half of that question, since a page pushed to the back is the only form of competition this
surface can observe.
...
- **Never implemented or field-removed** — these were documented but never read: `bloomq`, and `msaa`
  (field-tested and removed THEN: WebGPU allows `sampleCount` 1 or 4 only, and A2C needs 4 — **`msaa` came
  back on the dispatch console as 201/9-04's arm**, see the table above, and it is exactly those two values
  with the A2C loss stated rather than hidden), `ssr` and
