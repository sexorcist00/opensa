...
| **`msaa=` · `scene=`** | **201/9-04**: the scene pass's sample count (`1` or `4`) and colour format (`rgb10a2unorm` or `rgba16float`) — **48 bytes per pixel** of tile working set at the default, against the **16** Arm budgets for a 16x16 tile on the family the console's phone runs. Both are read into a `RenderBudget` the engine is CONSTRUCTED with, because every pipeline is compiled against them, so an arm is a page load rather than a key press. Each half falls back to the default independently, and `msaa=1` drops alpha-to-coverage from the cutout pipelines, which is a LOOK change the arm owes a verdict on |
| **`bloomlevels=`** | **201/9-05**: pin the bloom chain's level count instead of deriving it from the render size. The chain was the constant `8` — `1 + 8 + 7 = 16` full-screen passes whatever it drew into, the last three of them 11x10, 6x5 and 3x3 pixels, each still costing a whole pass's tile flush on a tile-based GPU. It derives now: **5 levels, 10 passes at the 720x640 the mobile circuit is pinned to**, 6 at 1920x1080, 7 at 3840x2160. `?bloomlevels=8` puts the old constant back, which is the A/B's other arm; an out-of-range value is CLAMPED rather than trusted (below two levels the composite binds nothing), so the report's `surface.bloomLevels` says what was built rather than what was asked for |
| **`cloudhz=`** | **201/9-06**: how many times a second the 256² cumulus field is re-baked; the default is **10 Hz** and `?cloudhz=0` bakes it every frame, which is what the console did before that step. The field has two inputs of different kinds — `cloudScale` STEPS (the weather changes it, and the field must change that frame) and time SCROLLS at `t * 0.004` (invisible frame to frame) — so the rule keys the first and amortizes the second, and a weather change re-bakes whatever the rate says |
| **why not `bloom=` / `clouds=`** | **Those two names are the GAME HOST's, and have been since 074/09** — `bloom` is bloom INTENSITY and `clouds` is cloud OPACITY, both in the table at the top of this file (`apps/web/src/ui/engine-canvas-host.tsx`, `apps/engine-lab`). 201/9-05 and 9-06 reached for them first and were renamed before either flew: the console and the game are two surfaces of ONE engine, and a parameter that means two things depending on which page read it is a capture nobody can re-read afterwards. It is SILENT — the pages are separate, so nothing collides at runtime and no test would have failed; what breaks is the record. `links.test.mjs` fails now if a console arm is spelled with either name |
