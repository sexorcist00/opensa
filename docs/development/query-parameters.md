...
| **`cad=0`** | **204/3-02**: remove the CAD STAND-IN — the fake that raises `cad` events (a panic, an assistance request, a plate read) so the bus can be heard and measured before PCAD ships the contract's `sound` field. It removes the stand-in ONLY: the seam it speaks through ([`cad-link.ts`](../../apps/dispatch/src/world/cad-link.ts)) is unchanged, which is exactly what the day a real CAD connects looks like. The capture says which it was (`audio.cad.online` is `true` when something has spoken, `null` when nothing ever has) and `audio.cad.assumed` counts the messages that named no sound and were played as `notification` — a CAD on the old wiring, where everything still chimes but every event chimes the SAME |
...
| `audio.panel` · `audio.cad` | **what the PANEL said (204)**: events played per category, folds, the worst **event → audible** gap against the 50 ms budget, and names no build knows; plus what arrived over the CAD seam and whether a CAD is answering at all. The latency is measured from when the event HAPPENED rather than from the call, because a message that took 200 ms to cross a socket and 1 ms to reach a speaker is a late alert |
