...
| `audio.arm` · `availability` · `clock` · `voices` · `absence` | **what could be HEARD (203/6-01)**: the arm, what the browser allows (`waiting` until a touch, `running` after one, `unsupported` where there is no Web Audio), the audio tick's own mean and worst against its 2 ms budget beside the rate it ran at, the voice pool's live/peak/steals, and every absence — counted exactly, the first 32 named. **A silent world and a broken one look identical from outside**, which is what these fields exist to separate |
