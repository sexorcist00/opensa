...
| `procobjFloor` | `0` (off) | number ≥ 0 | Species floor: while `procobjLimit` binds, every clutter MODEL eligible in the cell keeps at least this many placements instead of possibly none. Paid for at the top of the lottery order, so the budget is unchanged. `1` removes the defect entirely (17.7 % → 0 % of clutter cells losing a species) for 0.32 % of the drawn placements — [plan 012](../../tools/sa-procobj-placement/docs/plans/012-species-representation-floor.md) |
