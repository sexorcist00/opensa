...
| the game copy | `game-src/original` → `~/storage/downloads/New Folder`, which is **`/storage/emulated/0/Download/New Folder`** | holds `audio/`, `data/` and `models/` (audio confirmed 2026-09-09: `CONFIG` · `SFX` · `streams`, with `BankLkup.dat`, `PakFiles.dat`, `BankSlot.dat`, `EventVol.dat`, `StrmPaks.dat`, `TrakLkup.dat`). **CORRECTED 2026-09-09**: this row used to claim `~/storage/downloads` is a REAL directory inside Termux's private home rather than the usual `termux-setup-storage` symlink. It is the symlink. Two readings the same day: `ls -la game-src/` prints the link target `~/storage/downloads/New Folder`, and the panel's `GAME vs OUT` check — which is `fs.realpath`, so it follows every link — answers `/storage/emulated/0/Download/New Folder`. **The game copy is on SHARED storage, through FUSE**, which is the side the trap below is about |
