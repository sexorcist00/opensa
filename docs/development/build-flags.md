...
`peds.ide`/`vehicles.ide` roster and `fetch:pack` ships the whole build, so character/vehicle choice is
runtime-only (`GAME_CONFIG`) — no rebuild needed after changing them.
