...
| `procobj-species-floor.ts [--limit 150] [--density 1] [--stride 7] [--cells 200] [--rules <procobj.dat>] [--build-time]` | **does a clutter cap drop a SPECIES to zero** (plan 07/01's sizing task), through the shipping functions rather than a model of them. Default mode is the RUNTIME cell cap: on stock rules it binds in 97.9 % of clutter cells and zeroes ≥1 species in **19.8 %** of them (worst 14 of 25 placed); pointed at the BUILT tree's `procobj.dat` — 8 underwater rules, the rest converted to static — it zeroes **none**. `--build-time` runs `convertProcObj`'s whole-map pass per species (candidates → `lottery < 1` → MINDIST → `procObjMax`): nothing is zeroed at any stage, and MINDIST provably cannot (empty grid ⇒ the first placement of a batch always survives), though it is where the population goes (`sjmcacti2` 152 → 2 map-wide). Self-checks the cap identity — binding ⇒ exactly `limit` drawn — which caught the rig, not the engine, on the first run. **Read its corpus line**: geometry comes from `game-src`, so `--rules` is how a shipping-set question gets asked |
...
| `fx-anchor-census.ts [--pak <dir>] [--level hd\|lod] [--system <name>] [--near <x,y>] [--limit <n>]` | the 2dfx PARTICLE anchors a BUILT pak carries — per-system counts and, the half a count cannot give you, **where to stand to see one**. Plan 100's field check burned two rounds on emitters it had no coordinate for. Canonical pak (`13:19 08-08-2026`): **943 anchors in 569 hd cells**, `insects` 336, `vent` 209, `vent2` 162, `cigarette_smoke` 87, `fire` 53, `smoke30lit` 49; the lod level carries 888 of them, so **do not sum the levels**. These are NOT the 878/`insects` 402/`cigarette_smoke` 0 the 100 ledgers quote — those came from an in-process bake count, this is read off the shipping bytes. Prints anchors in GTA coords, and **self-checks that each one lands in the cell it was read from**: the pak stores them cell-LOCAL, and reading them as world put 934 of 943 up to 3 km away while printing perfectly plausible numbers |
