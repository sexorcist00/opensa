...
| `cleo-place-mods.ts` | hand-place the two class-A CLEO corpus mods (Ferris + Wind Farm) into `build/original/opensa` for a field run: scripts → `cleo/`, IDEs → `data/maps/` + `gta.dat` lines, models → a `models/cleomods.img` override. TEMPORARY: plan 097/06's packaging retires it — until then this is how a `?cleo=1` field session gets its mods |
