...
| `video-accept.ts <harness.log>…` | video mode's acceptance exam, rolled up off a harness log — the numbers every 096 phase re-sits, so a change is judged against the same question the last one was: scenes, directed frames, the share inside the safe frame, cuts and what ended them, how often the pan cap bit, `[cam] jump` lines, the realised mod-car share, and the regions IN THE ORDER THEY PLAYED (the cycle is an acceptance question a log can answer). Dedupes the report lines the harness echoes twice, and weights each scene's `safe` share by its own frame count rather than averaging shares |
