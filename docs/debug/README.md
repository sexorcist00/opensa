...
| `txd-retune.ts <mod.txd> [--match <txd\|entry>] [--add <txd>#<name>] [--max n] [--no-mips] [--write]` | **rewrites** a mod's dictionary: every texture resized to the dimensions a REFERENCE (normally the stock) dictionary carries, plus textures an earlier mod ADDED that this one would silently drop. Dry-run by default; prints the plan and the bytes saved. Two real cases it exists for: an HD retexture that cost 5.6 MB against a stock 106 KB (mod 56 → −95 %), and mod 4 overwriting mod 3's `cj_commercial.txd` so `cj_frame_glass2` vanished and mod 3's fridge lost its glass (the pak already reports these in `report.json` → `textures.missing` — `--add` is how you put one back). **Gotcha:** a reference with a different ASPECT cannot be reached by halving; the tool stops at the nearest step above it and says so rather than squashing what the author drew |
