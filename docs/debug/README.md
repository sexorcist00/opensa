...
| `alpha-class-census.ts [--img <path>] [--flips] [--json <out>] [--below/--above/--near <share>] [--txd <substring>]` | the alpha CLASS of every texture in an archive — which ones the pack sends to the depth-writing cutout pass and which land in the blend pass, **where the background paints over the foreground** (plan 092). Reports both readings of the histogram: the current absolute one (`mid ≤ 2 %` ⇒ cutout) and the candidate one relative to vanilla's ~128 alpha test (`below` / `near` / `above`), plus the flip list a candidate rule would produce. Defaults to `build/<game>/sa/models/gta3.img` — the merged, mods-installed tree. The thresholds are CLI flags on purpose: the class is BAKED, so the rule is fitted on the census instead of on a re-pack |
