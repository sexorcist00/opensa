...
| [audio-stream-tracks.md](audio-stream-tracks.md) | a zone with no assembled ambience bed, or a bed an ear calls thin — `AmbienceHost.noLayers` in `packages/audio/src/ambience.ts` names this card, and the console's wiring of it reaches a capture through `audio.absence.reasons` |
