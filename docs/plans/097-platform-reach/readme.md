...
| 3/01 bake collision into the pak | **PART SHIPPED 2026-08-04** — the `.oscol` container and the bake, with the runtime's own `toModelColliders` as the bake's test oracle; the pak entry kind and `--bake-collision` now write it; the RUNTIME READ is still to come, and with it the before/after. The 250-vs-256 grid trap is now a recorded restriction |
| chains 2, 4, 5 | pending (2 and 5 wait on their concepts) |
