...
| 3/01 bake collision into the pak | **PART SHIPPED 2026-08-04** — the `.oscol` container and the bake, with the runtime's own `toModelColliders` as the bake's test oracle; the pak entry kind, the converter wiring and the runtime read are still to come. The 250-vs-256 grid trap is now a recorded restriction |
