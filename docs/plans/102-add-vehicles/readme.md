...
## Why this lives in the central folder

It spans four homes and the pipeline, and each piece is shippable alone:

| chain | what | status |
| --- | --- | --- |
| [`tools/vehicle-installer/docs/plans/012`](../../../tools/vehicle-installer/docs/plans/012-unread-file-kinds.md) | `model-variations-extra.txt` + `text.txt` read at last; the settings-fallback trap; `.fxt` + ModelVariations merges are born here | planned |
| [`tools/vehicle-installer/docs/plans/013`](../../../tools/vehicle-installer/docs/plans/013-audio-and-parked.md) | `audio.txt` + `parked.txt` for replacement cars; FLA audio cfg + Parked Maker merges are born here | planned |
| [`tools/add-vehicles/docs/plans/`](../../../tools/add-vehicles/docs/plans/readme.md) `001`–`007` | the new tool: resolver root, ids + rows + IMG, name/sound/parking, traffic, derived tuning, tuned traffic, pipeline + field | planned |
