...
| **1/01 The format check** | **NOT STARTED, and it is the NEXT WORK.** It needs the phone, which has not answered this session; every number in the concept's §1 is documentation until it runs. **No step below it may start on an unverified number** |
| 1/02 … 6/02 | not started |

**What this chain owes before it can claim anything**: a device row for 1/01, and then the three numbers
3/03 asks for. Nothing here has been measured, and no line of this plan has been built.
