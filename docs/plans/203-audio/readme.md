...
| 2/01 … 2/03 · 3/02 … 6/02 | not started |

**What this chain owes before it can claim anything**: the 1/01 run on a tree that HAS an `audio/` folder.
The instrument is written, registered in [the debug index](../../debug/README.md), proven in both directions,
and it has now been run — it answered that the phone's game copy carries no audio at all (2026-09-09). So the
one thing between this chain and its gate is the folder:

```bash
# on the phone, into the game copy the panel already points at
cp -r "<the game's audio folder>" "/storage/emulated/0/Download/New Folder/audio"
```

Nothing about the real files has been measured yet, and **no number in the concept's §1 may be cited until
that run says `LAYOUT AGREES`**. **3/01 is the one step that may be built anyway** — an `AudioContext` and the
autoplay gate know nothing about bank offsets — and it was, on 2026-09-09; everything else waits.
