...
| **3/01 `map`: the board's own lifecycle** | **DONE 2026-09-11.** `apps/dispatch/src/world/board-events.ts` diffs two `Operations` snapshots into event names — a call appears (coded by its priority, because the visuals carry it three ways and so must the sound), a unit commits, a unit reaches its scene, a call clears. **A diff rather than a subscription**, which is what makes it a pure function two objects can be handed to; the console renders a board it does not own, so there is no event stream to subscribe to in the first place. **It runs on the AUDIO clock, ten a second**, for the same reason the listener does: the board moves whether or not a frame was drawn for it, and the render gate takes the frame rate to zero at rest. **The FIRST board raises nothing** — every call on it is already there rather than newly arrived, and a console that announced its whole roster at open is one somebody mutes in the first minute. A unit appearing mid-shift is coming on duty, which the contract gives no sound and the code names as considered rather than missed. 13 tests on the diff, 21 on the console's audio, 455 across the app and the packages. Two mutations fail: never remembering the board, and diffing a board against itself |
| **3/02 `cad`: the seam, and a stand-in on it** | **DONE 2026-09-11.** [`cad-link.ts`](../../../apps/dispatch/src/world/cad-link.ts) is the console's end of the CAD link: a message in the contract's shape becomes a panel event. **The sound is the `sound` FIELD and never the title's text** — PCAD picks its sound today by searching the title, so rewording, translating or fixing a typo in it stops the sound silently with the notification still appearing. A message that names nothing plays `notification` and is COUNTED, because *the CAD stopped naming its sounds* and *the CAD went quiet* look identical from here, and the first is the worse one: everything still chimes and every event now chimes the same. A blank field counts as absent rather than as an unknown NAME, which would file a wiring problem in the vocabulary column. **`link_lost` / `link_back` are DERIVED**, for the reason a dropped link cannot send anything, and neither is said until a CAD has answered once — the agent link's own rule, where a state claimed before anyone answered is a claim rather than a reading. **[The stand-in](../../../apps/dispatch/src/world/cad-mock.ts) plays PCAD's part through that same seam** (decision 3.3), so the `cad` bus's ducking, its reserve and its 50 ms budget are measurable before another repository ships: a message roughly every 45 s about a unit actually on the board, `panic_button` the rarest of the seven because a demo that fires it every third message teaches an operator to ignore it. It always carries the `sound` field — a mock that sometimes omitted it would be this repository manufacturing the defect it is asking the other one to fix — and it never fakes a DROP. `?cad=0` removes the stand-in and leaves the seam exactly as it is, which is also what the day PCAD arrives looks like. 9 tests on the link, 6 on the stand-in, 25 on the console's audio, 854 across the app and the packages. **A guard was deleted rather than kept**: `gapSeconds <= 0` cannot fire, because a random in [0, 1) is never `>= 0` false at a zero threshold — the comment now says the arithmetic already refuses it. **And one mutation survived and cost a test**: dividing the gap by the mean was replaceable by the gap itself and every test passed, because the rate was never pinned — it is now asserted at the threshold from both sides, which is what makes *the same mock sounds the same at 10 Hz and at 2 Hz* a fact |
| **3/03 Every event is also SEEN** | **DONE 2026-09-11.** [DESIGN.md](../../../apps/dispatch/DESIGN.md)'s redundancy rule applied to sound — *any one channel read alone is enough* — and it is also the defence of sound being ON by default: it ADDS a channel rather than carrying one alone. **The finding is that `map` needed nothing**: a call appearing, a unit committing, a unit arriving and a call clearing are all already drawn by the board, and a second line over a queue that just grew a row is chrome covering what it reports. What had NO visual at all was `cad`, because nothing on the console knows those events except the sound they made — so [`panel-notices.tsx`](../../../apps/dispatch/src/ui/panel-notices.tsx) gives each one a line, held **4 s**, **15 s** for the two FLOORED names in the danger style. That is one rule and not two: the same two events a mute leaves audible are the two the screen holds longest, because a `cad` event is not on the board and nothing else on this console remembers it. **The wiring had to move, and that was the real work.** The CAD link and the stand-in were built inside `DispatchAudio` in 3/02, which made the SCREEN depend on the speaker three ways: `?audio=0` never starts the audio tick, so the stand-in never ran; plan mode — the surface every non-WebGPU browser falls back to — has no audio object at all; and a throwing sink would have taken the rest of the fan-out with it. The link now belongs to the console ([`use-cad.ts`](../../../apps/dispatch/src/ops/use-cad.ts), beside the board), both channels subscribe, a sink that throws is COUNTED rather than rethrown, and the capture's `cad` block moved out from under `audio` for the same reason — a report that filed these under the speaker would say a muted console received nothing. `?cad=0` and `?audio=0` are two arms read from two parameters, so neither can remove the other. 4 tests on the hold, 10 on the link, 3 on the words, 860 across the app and the packages; the floored-hold mutation and the throwing-sink case both fail without the code |
| **3/04 The backgrounded tab** | **DONE 2026-09-11** (the device half owed to 5/01). The dispatcher is a player, so this console spends its shift behind a game window where a browser clamps a hidden tab's timers to about 1 Hz — and the answer is a DIVISION rather than a fix, written into [the contract](../../contracts/panel-audio.md) §4b as a table. **The `cad` path touches no timer at all**: `event()` is a lookup and a voice, and the test that proves it never starts the clock. What IS throttled is stated and accepted — the ambience's positions (203 already said so; the graph keeps playing in the audio thread), the `map` diff (the board it compares is stepped on a React timer clamped by the same rule, so the diff cannot be later than its own inputs), and the stand-in, which is a fake: a real CAD arrives on a socket, and a socket wakes a hidden tab. **And a SUSPENDED context is asked back on the way past**, because a tab nobody is looking at gets no gesture and the alert is precisely what would be waiting for one; the refusal path is the existing counted one rather than a throw, since a context that returns mid-tone is a late alert and a dropped one is no alert. 23 tests on the console's audio, 862 across the app and the packages; removing the resume fails the suspended-tab test. **Owed**: whether Android actually keeps this context alive behind the game window is a question only the phone can answer, and it goes in 5/01's row |
