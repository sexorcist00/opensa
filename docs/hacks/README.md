...
| [Cutscene window-pane suppression](cutscene-window-pane-suppression.md) | `vehicle-cutscene/census.ts` + `rig/emit.ts` | control over a scene's entity DRAW ORDER, which the 2004 cutscene path does not offer — a rendered window pane z-writes and erases actors drawn after the car, so field-losing slots (`csgreenwood`, RIOT_4B) drop their whole window class like R*'s own glassless cutscene cars; retired by an order/z-write hook or OpenSA's cutscene player |
