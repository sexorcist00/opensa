...
| [`procObjMax` as clearance](procobj-max-as-clearance.md) | `tools/lod-procobj-generator/src/config.ts` | a MEASURED clutter budget — 100 000 is picked so it cannot bind (the layer places 91 092), because the real number is 013's perf budget and no measurement exists yet. The old 20 000 silently became the thing every density experiment measured |
