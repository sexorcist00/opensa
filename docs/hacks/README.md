...
| [New tuning part's `0x200000`](retired/upgrade-part-no-collision-flag.md) (RETIRED 2026-08-20, the day it was taken) | `tools/vehicle-installer/src/tuning-parts.ts` | a collision model for a part the stock game never had. Taken on a reading of the IDE flags the field disproved in two crash logs the same day; **retired** by generating a bounds-only COL3 per part into `models/coll/opensa-parts.col`. The evidence for it was 46 parts nothing had ever spawned — a control group never exposed to the treatment |
