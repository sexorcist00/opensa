...
| [sa-target.md](sa-target.md) | What the `sa/` target must budget for before a plan adds anything to the map, and — for anything that PATCHES the real exe — that an address or expected byte is declared once, in the catalogue, never restated in a patch (a trampoline's continuation is derived, not written) |
