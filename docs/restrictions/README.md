...
| [sa-target.md](sa-target.md) | What the `sa/` target must budget for before a plan adds anything to the map, that a ceiling's guard belongs on the BRANCH whose target has it (a shared-stage guard silently rations the target without the limit, and one does today), and — for anything that PATCHES the real exe — that an address or expected byte is declared once, in the catalogue, never restated in a patch (a trampoline's continuation is derived, not written) |
