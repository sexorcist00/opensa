...
| **Vehicle colour palette** — `carcols.dat`'s `col` section into `CVehicleModelInfo::ms_vehicleColourTable` | **128** (FLA's stated default) | **unchanged — the setting was tried and REVERTED 2026-08-19**. The install has been running 140 rows with it untouched, so the 128 is FLA's annotation and not a watched ceiling; raising it pulls in a uint32 colour-id patch family for headroom nobody has proven is needed | past the table the loader writes into whatever follows it. Stock ships 127, so a mod set has 1 row of headroom before it is over. **Caught: WARNED, not refused** — `vehicleColourWarnings` on every install/rebake path prints the count against the limit; it is not a hard guard because the 128 is inferred from the adjuster rather than read out of the exe, and the build has been running past it with no reported symptom ([measurement](../gta-sa-original/vehicle-colour-table-128.md)) |
