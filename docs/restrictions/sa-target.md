...
| **`carmods.dat` `link` pairs** — `CLinkedUpgradeList` arrays | **30, game-wide** | **unchanged — no adjuster has a setting; `asi/perfect-vehicle` plan 002 lifts it** | the 31st pair writes past the arrays (into `m_nLinksCount` and on) — silent static corruption. Stock uses 23, every added car that re-models its base's wings costs 1. **Caught: by `tools/add-vehicles` (refuses above 30 and names the plugin) once it ships; before that NO** ([measurement](../gta-sa-original/carmods-upgrade-ceilings.md)) |
| **Parts on one car's carmods line** — `CVehicleModelInfo::m_anUpgrades[18]` | **16 listed** (+`hydralics`+`stereo` appended always) | **unchanged — `asi/perfect-vehicle` plan 002 lifts it** | the 17th part overruns the per-car array into the model info's next fields. Stock `jester` is full at 16. **Caught: `tools/add-vehicles` guard once it ships; `vehicle-installer`'s carmods merge does NOT count today — a replacement car's line above 16 is silent** |
| **Vehicle colour palette** — `carcols.dat`'s `col` section into `CVehicleModelInfo::ms_vehicleColourTable` | **128** (FLA's stated default) | **256 since 2026-08-19** — `Vehicle colors` was COMMENTED in the install while the build already carried 140 rows; the user set it to 256 (a colour is a byte in the save). NOT yet field-confirmed | past the table the loader writes into whatever follows it. Stock ships 127, so a mod set has 1 row of headroom before it is over. **Caught: WARNED, not refused** — `vehicleColourWarnings` on every install/rebake path prints the count against the limit; it is not a hard guard because the 128 is inferred from the adjuster rather than read out of the exe, and the build has been running past it with no reported symptom ([measurement](../gta-sa-original/vehicle-colour-table-128.md)) |
...
| **A tuning part's NAME** | prefix decides its component flags; ≤ 19 chars (IDE `sscanf` into `char[24]`, IMG name 24 bytes incl. `.dff`) | unchanged | a renamed part without its stock prefix loses its behaviour silently; a long name smashes the IDE loader's stack. **Caught: `tools/add-vehicles` derives names prefix-first and refuses > 19** |
