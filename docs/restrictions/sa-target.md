...
| `CPool<CDummy>` — an `inst` row whose model is in `object.dat` | 2,500 | **`Dummys = 100000`** (OLA) — raised from 50 000 in the field 2026-08-19. **Stock places 40 of these; our build places 17 539**, 17 311 of them the procobj bake | the TWIN of the row above, at `0x00538103` — the pool allocator returns null and `LoadObjectInstance` dereferences it. **Caught: NO** — the build prices every text row against `CPool<CBuilding>` and prices this pool not at all |
| **`IplDef.firstDummy/lastDummy`** — the range `RemoveIpl` frees dummies by | **32,767 (int16)** | **NOT LIFTED** — `perfect-map.asi` patch #1 lifts only the BUILDING pair (004b) | a dummy at pool index > 32 767 is **never freed**, so raising `Dummys` only hands the leak more slots — the pool buys `floor(Dummys / 17539)` world entries per boot and nothing else (**field-confirmed 2026-08-19 in both directions**: 50 000 died on the 3rd entry, 100 000 on the 6th). **There is no safe configuration**: capping `Dummys` at 32 767 to stay inside the field does not BOOT (field 2026-08-19, 15 s, before the menu), so the truncated range is walked on every run the field can actually start. **Caught: NO, and it is invisible** — the crash arrives on a later world entry, not at the placement that caused it ([open issue](../open-issues/sa-load-game-crash-dummy-pool.md)) |
...
| A DFF's frame list ORDER | a parent must precede its child | **unchanged — nothing lifts this** | RenderWare parents each frame in the pass that creates it, so a forward parent index reads an unwritten slot: crash in `RwFrameAddChild` (`0x007F0BF7`) or silent corruption, decided by leftover memory. **Caught: NO** — byte-faithful conversion ships a mod's file as authored, and OpenSA's own reader is index-based so it never notices ([the fact](../gta-sa-original/rw-frame-list-parent-order.md)) |
