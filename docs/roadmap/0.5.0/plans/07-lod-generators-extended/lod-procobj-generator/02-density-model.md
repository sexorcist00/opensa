...
**And the wall is not an artifact of how we split areas** — the obvious objection, checked against the mod on 2026-08-08 rather than argued from this file. An area's text rows AND its binary streams share SA's one 4 096-slot `LoadScene` buffer, so an area holds ~2 000 of our objects (two rows each: a permanent LOD row plus a binary HD row, or two binary rows when unlinked). Stock has ONE free slot. ProperFixes' 3.77× does not disprove it: its six 9 600-row files are 2.4× that buffer and work only because its own readme REQUIRES Open Limit Adjuster; priced legally at 4 096 rows/file even its LOD-less layout needs **15 slots** for 57 583 objects. Nobody reaches the target on stock. The one lever that would move it is the ~30 slots the base map itself spends, which nothing has costed — see [density-target.md](../density-target.md).
