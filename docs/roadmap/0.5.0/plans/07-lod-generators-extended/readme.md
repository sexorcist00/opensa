...
| **P2** | [lod-procobj-generator/04 — slot economy & budgets](lod-procobj-generator/04-slot-economy-and-budgets.md) | **Rewritten PER TARGET 2026-08-08**, and it stops being a slot-economy plan: with stock dropped as a target, `EntityIpl`/`EntitiesPerIpl` are `unlimited` where it matters, so slots are not a currency and int16 is the only correctness ceiling left — ours to clear. Its first task landed 2026-08-08 — the target selector (pmb `--target`, derived from `--exclude`), with the layer's price now reported per host. Its second is still not raising a cap: `checkTextIplSlotBudget` runs on the COMMON baked build, so an opensa-only build is refused past 30 000 text rows by a ceiling its engine does not have. Now a [restriction](../../../../restrictions/sa-target.md). Perf is the budget on two hosts, and a budget names its host. |
