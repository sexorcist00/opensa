...
7. **And stock SA is not a target at all (2026-08-08, the user's call), which retires half of finding 5.**
   The declared configuration is OLA + FLA plus our own `perfect-map.asi`
   ([`gta-sa-original/reference-install.md`](../../../../gta-sa-original/reference-install.md)), so ~~the
   build guards are right to keep enforcing the stock story~~ is wrong: the slot array and the per-file
   buffer are `unlimited` where we ship, and a guard that fails a supported build to protect an unsupported
   one is the wrong trade. **int16 is the only correctness ceiling left, and it is ours** — no adjuster
   lifts it. Findings 2 and 3 stay as the record of what stock costs; nothing plans against them.
   [02](lod-procobj-generator/02-density-model.md) and
...
| P | Plan | Why here |
| --- | --- | --- |
| **P0** | [lod-procobj-generator/00 — limit route review](lod-procobj-generator/00-limit-route-review.md) | A decision, no code. **Largely ANSWERED by finding 7**: stock is not a target, so the route is OLA + FLA for the pools and `perfect-map.asi` for int16 — the one ceiling no adjuster lifts. What is left of it is the asi's own shipping/verification story, not a choice of ceiling. |
...
| **P2** | [lod-procobj-generator/02 — density model](lod-procobj-generator/02-density-model.md) → [03 — biome density](lod-procobj-generator/03-biome-zone-density.md) | **Rewritten PER TARGET 2026-08-08** (the user's call), then narrowed to TWO the same day — **stock SA is not a target** (the declared configuration is OLA + FLA + our asi). The machinery is shared, every number is not: `sa` reaches the full 3.77× above an int16 gate that only `perfect-map.asi` clears, `opensa` has no SA ceiling at all. A profile declares its target and its asi gate, and the build refuses a mismatch at config time. |
| **P2** | [lod-procobj-generator/04 — slot economy & budgets](lod-procobj-generator/04-slot-economy-and-budgets.md) | **Rewritten PER TARGET 2026-08-08**, and it stops being a slot-economy plan: with stock dropped as a target, `EntityIpl`/`EntitiesPerIpl` are `unlimited` where it matters, so slots are not a currency and int16 is the only correctness ceiling left — ours to clear. Its first two tasks were not raising a cap and **both landed 2026-08-08**: the target selector (pmb `--target`, derived from `--exclude`) and the guard move — `checkTextIplBudgets` now runs on the built `sa/` tree with int16 throwing and the 39 stock slots reporting, while `opensa/` runs neither and announces that it has no ceiling of its own yet. Now a [restriction](../../../../restrictions/sa-target.md). What is left: the two perf budgets, and a budget names its host. |
