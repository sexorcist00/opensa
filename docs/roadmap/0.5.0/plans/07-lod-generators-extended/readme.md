...
| **P2** | [lod-procobj-generator/02 — density model](lod-procobj-generator/02-density-model.md) → [03 — biome density](lod-procobj-generator/03-biome-zone-density.md) | **Rewritten PER TARGET 2026-08-08** (the user's call): the machinery is shared, every number is not. Three profiles — `sa-stock` is a REDISTRIBUTION (1.18× is the whole of its headroom), `sa-reference` reaches 3.77× above an asi gate, `opensa` has no SA ceiling at all. A profile now declares its target and the build refuses a mismatch at config time. |
| **P2** | [lod-procobj-generator/04 — slot economy & budgets](lod-procobj-generator/04-slot-economy-and-budgets.md) | **Rewritten PER TARGET 2026-08-08.** Three targets, three limiters, and its first task is not raising a cap: `checkTextIplSlotBudget` runs on the COMMON baked build, so an opensa-only build is refused past 30 000 text rows by a 2004 int16 ceiling its engine does not have. Now a [restriction](../../../../restrictions/sa-target.md). Perf is the budget on two hosts, and a budget names its host. |
