...
(contract first — the seam is already named in `apps/dispatch/src/ops/sim.ts`, and there is no networking
anywhere in this repo yet), **real routes** instead of bearings (blocked on the `original`-only vehicle node
graph), **replay/history**, **multi-operator**, and **install + offline**. Deferred on 2026-08-06 by decision
