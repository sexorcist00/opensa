...
| [vehicle-colour-table-128.md](vehicle-colour-table-128.md) | **`carcols.dat`'s palette is a 128-row table and the shipping build carries 140** (2026-08-19): stock has 127, the mod fleet added 13, and three of those are DUPLICATES — `addPaletteColors` appended a mod's colours on every re-run, so the palette grew with the number of rebakes rather than with the mod set. Fixed (a colour is reused when RGB + description match); the overflow itself is WARNED, not refused, because the 128 comes from FLA's stated default and not from a disassembly. Raising `Vehicle colors` in the FLA ini is the free fix and is the user's call |
