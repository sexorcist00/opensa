...
- <https://gtamods.com/wiki/SFX_(SA)> — **the SFX bank format**, exact enough to address a single sound by
  arithmetic: `BankLkup.dat`'s 12-byte entries, `PakFiles.dat`'s 52-byte names, the 4 804-byte bank header
  ...
  [the audio concept](plans/203-audio/concept.md), where every number is owed a check against the real files.
