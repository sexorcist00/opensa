...
| **09-05** | **[mobile-units-as-models-first](opensa-engine/2026-09-05-mobile-units-as-models-first.json)** | **THE FIRST CAPTURE IN THIS REPOSITORY WITH UNITS DRAWN AS MODELS.** 201/5-02 and 5/04 have owed it since the budget was written and it was never takeable: every pak the console ever ran was converted with `MODELS=0`, so `unitsAsModels` read 0 on every capture. The blocker was never code — the path resolved names, opened archives over Range requests and logged `this build carries no model of that name` exactly as designed. Pak `07:40 05-09-2026`, built this session into its OWN OUT (`MODELS=1 BAKE=1`, 5 vehicles, collision baked) so the pak the rest of the 09-05 series was measured on survives | **A FIRST LOOK, NOT A ROUTE ROW, and filed apart from the 09-05 series on purpose**: new pak, ten-leg route NOT flown (the camera drifted under a hand), 260 moving frames against the 300 the collector asks for, and three model types rather than the five the board asks for since this session. What it IS good for is the structural counts, which are pose-independent | **12 197 DRAWS against the empty map's 96 — 127x, not the 'order of magnitude' the frame audit predicted hours earlier — and 1.3 M triangles against 242 k.** At 9 units for scale: 837 draws, i.e. **~80 per car**, which is submeshes, because a vehicle is a part hierarchy rather than a mesh · `modelTextureMb` is the SAME 6.29 at 9 units and at 150, which is `ops/budget.ts`'s type-count axis doing exactly what it says · **and the second finding is the SYMBOLOGY**: `overlay-2d` is **6.17 ms of an 11.65 ms CPU body**, twice `engine-frame`'s 3.12 — the 2D layer, not the 3D one, is where the CPU goes at the declared count, which is 5/02's owed subtraction seen from the inside · the worst frame is `board` 49.1 ms + `engine-frame` 34.6 · **it moves the frame audit's ranking**: its GPU-driven entry stops being 'interesting only at that load' and becomes the load, with instancing the unit models ahead of a culling pipeline |
| **09-05** | **[desk-canvas-symbol-arms](opensa-engine/2026-09-05-desk-canvas-symbol-arms.json)** | **A DESK CONTROL, NOT A DEVICE ROW**, and it says so in its own file. 201/9-01 bakes each symbology mark once and blits it per instance instead of rebuilding its path every frame (MapLibre's icon atlas, deck.gl's `IconLayer`); the phone has no `timestamp-query`, so the device arm is `nosprites` against `board` and needs a settled window and a thermal bracket. This answers the narrower question in ten seconds: is a blit cheaper at all. `scripts/debug/canvas-symbol-arms.mjs`, headless Chromium, 720x640, 150 marks, arms INTERLEAVED and the whole run repeated three times | **THE INSTRUMENT FAILED FIRST AND IS RECORDED FAILING**: `performance.now()` is clamped to ~100 us and one frame of 150 marks is 0.3 ms — three ticks — so the first version quantized every sample onto 0.3 / 0.4 / 0.5, the means overlapped, and it read as a clean 15 % win it could not have resolved. A sample is a BLOCK of 40 frames now, divided back down | **blit ~0.275 ms against path ~0.32 ms for 150 marks** — the cheaper arm in all three paired invocations, ~10-20 %, and the path arm's second sample is always below its first (warm-up) while the blit arm does not move, so the honest number is the low end · **the second reading is the one that matters**: 150 marks cost ~0.3 ms HERE while the phone reports `overlay:symbols` at **7.37 ms** at 192 symbols, so on the device the DRAWING is a minority of that span and the rest is what the layer does per unit around it — two projections, a colour string, a cache key, a hit-area object and a rank object, 150 times a frame (**all three allocations removed the same evening** — see [per-unit-allocation](opensa-engine/2026-09-05-mobile-per-unit-allocation.json)) · **EXTENDED 09-05 evening with the SCALE BAR**, which the decomposition had left explained by a guess: `sym:scale` is ~250 µs a frame on the device for ONE bar, and the obvious cause was its two `ctx.font` assignments. **Refuted as an absolute explanation** — alternating two shorthands costs **0.6 µs** an assignment against 0.1 for repeating one (so the churn IS ~6x, exactly as this layer's oldest note says, and it is 1.2 µs a frame) and the **whole bar is 3 µs**. The device spends ~250 µs on 3 µs of desk work, the same ~80x ratio `sym:units` carried before its allocations went — and **no change was shipped on the refuted guess** |
| **09-05** | **[mobile-symbol-sprite-arm](opensa-engine/2026-09-05-mobile-symbol-sprite-arm.json)** | 201/9-01's sprite change flown on the DEVICE against its own control, six windows, arms alternating: `board` (each mark rasterized once and blitted) against `nosprites` (`?sprites=0`, the layer's own path fallback — what shipped before). Pak `07:40 05-09-2026` (`MODELS=1 BAKE=1`), app **`f0f0336+`**, `?surface=720x640` on every window, ONE settled pose `[1500,-1500] h900 pitch −1.15 yaw π` held identical throughout — not the ten-leg route, because this is a CPU span the camera path does not change and holding the pose removes a variable | **MGA-LX3 / ARM Bifrost @ DPR 2**, board at the declared 150 units + 40 calls. **The arm is proven NON-NULL in every window**: `symbology.spriteVariants` reads 0 on the control against 6–9 on the sprite arm. Windows 4–6 carry the fleet loaded throughout (`unitsAsModels` 150); `drawUnit` draws its chevron either way, so the span is the same work at 0 and at 150 | **INDISTINGUISHABLE.** board **7.014 / 6.162 / 5.971** (range 1.04 ms), nosprites **6.429 / 6.103 / 6.542** (range 0.44) — the sprite arm's own spread SPANS the control's, and the sign flips with the pairing: windows 1–2 read −0.58 ms for the control, windows 5–6 read +0.57 for the sprite arm · **the first four windows fall monotonically while the arm alternates** (7.014, 6.429, 6.162, 6.103), which is warm-up, and stopping at windows 1–2 would have filed a confident −0.58 ms result — the third time in this chain that three windows agreed on a false story · **the desk control predicted exactly this**: it put the whole effect at 0.03–0.06 ms per 150 marks against a session spread of ~1 ms · **and the finding that matters is what it rules OUT**: 150 marks cost ~0.3 ms of canvas work at a desk, so the drawing is one or two of these six milliseconds and the rest is the layer's per-unit work AROUND it — two projections, a colour string and a cache key built per unit per frame, a hit-area object, a rank object, 190 times a frame. `symbology.render` needs the same split `overlay-2d` just got |
| **09-05** | **[mobile-symbology-split](opensa-engine/2026-09-05-mobile-symbology-split.json)** | **THE 6 ms ATTRIBUTED.** `overlay:symbols` was the frame's largest CPU line all day with no breakdown at all — the same shape `overlay-2d` was in that morning, and the same fix one level down: four spans matching the passes (`sym:calls`, `sym:units`, `sym:labels`, `sym:scale`), the recorder passed in by the host so plan mode and every test pay nothing. App **`adc8182+`**, same pak, pin and settled pose as the sprite arm | **MGA-LX3 / ARM Bifrost @ DPR 2**, 438 frames settled, 150 units + 41 calls, **every mark drawn** (`unitsAsModels` 0 in this window, so the rule below hid nothing and this is the honest before) | **`sym:units` 4.07 · `sym:labels` 1.57 · `sym:calls` 0.77 · `sym:scale` 0.25**, summing to **6.66 ms** — inside the 6.0–7.0 range the six sprite-arm windows measured for the whole span, so the split is faithful rather than an added cost · **the units pass is 61 %** and is where further work goes · **a unit is 27 µs and a call 19 µs while a unit does roughly TWICE the drawing** — a cost nearly flat per entity while the drawing per entity is not is a cost that is mostly not the drawing, which is the desk control's finding reached from the other side · **`sym:scale` is 0.25 ms for ONE scale bar**, a third of what forty calls cost, and it assigns `ctx.font` twice (this file's own oldest note is that a font assignment re-parses the shorthand) · **the baseline was flown TWICE** — 4.07 and 4.18 for the units pass with every mark drawn, a spread of 0.11 ms, which is what makes a subtraction from it worth taking · **the operator's mark rule is IN and fires** (`marksHidden` 108–110 of 150 with the fleet loaded) and its first number is **`sym:units` 3.21 ms** — but that window is MIXED, the fleet arrived partway through it, so 3.21 is an UPPER BOUND rather than the value. Against a pre-registered 1.5–2.0 ms it says at least ~0.9 and does not yet reach the estimate; a mixed window is not evidence against one. **Still owed**: a settled window with the fleet loaded throughout, or a `?marks=all` arm differing by the rule alone · **the rule was extended to the BEACON layer the same day** at the operator's word, trails included — the screenshot between the two changes is why: no chevrons, and 150 green crosses with tracks across the whole city, because two layers drew the same fact and only one had been filtered |
| **09-05** | **[mobile-vehicle-instancing](opensa-engine/2026-09-05-mobile-vehicle-instancing.json)** | **201/9-08 — the unit models instanced**, the frame audit's top item taken on the device. A vehicle is a part hierarchy, so every part was its own draw PER CAR; opaque order is decided by depth, so a run of consecutive slots drawing the same submeshes is one draw per submesh however many cars are in it. The per-instance buffers stay slot-major (the write side sends a whole car's matrices in ONE `writeBuffer`), so the draw passes the SLOT through `firstInstance`, names its PART in a second dynamic-offset uniform, and the shader computes `slot × partCount + part`. App **`8cda1fd+`**, same pak, pin and settled pose | **MGA-LX3 / ARM Bifrost @ DPR 2**, 150 units + ~40 calls, fleet loaded (`unitsAsModels` 150, 5 types). **`multi-draw indirect` and `bindless` are WebGPU PROPOSALS**, so the one-call-for-everything shape is unavailable and this uses plain instanced `drawIndexed` — which is what the audit named as the honest first move | **draws 11 810 → 3 571 (−69.8 %), triangles IDENTICAL at 1 488 514**, residency unchanged. Exact rather than sampled: a draw count is a count · what remains is mostly the TRANSLUCENT phase, which is not instanced and is not asked to be — its order is a function of the eye and differs per car (074/16 round 6), so glass stays one draw per car per submesh at ~21 a car · **no frame-time claim is made**: the windows either side were hours apart on a device that had run all session, the CSS box differed (360x320 against 360x609), and the day's fleet-loaded `dtMean` spans 26.8–59.9 ms with nothing changed · **AND THE FIRST ATTEMPT SHIPPED INERT** — the run key asked *is nothing hidden*, and nothing ever is (`apps/dispatch` hides every submesh and re-shows the body set; the game's handle does the same for extras and damage), so every car was drawn alone and the device returned **11 810, unchanged to the unit**. It typechecked, it linted and seven new tests passed: they exercised the mechanism rather than the workload. The set is what groups, and `opaqueOrder` already was it |
| **09-05** | **[mobile-board-decomposition](opensa-engine/2026-09-05-mobile-board-decomposition.json)** | **WHAT THE FRAME IS AND WHERE IT GOES** at the declared board after 9-08's instancing. Four settled windows in one session, arms alternating, the baseline flown TWICE — and the fourth arrived by accident (the models did not finish loading) and turned out to be the arm the set was missing: `board` minus the fleet, symbology intact. App **`8cda1fd+`**, same pak, pin and pose in all four | **MGA-LX3 / ARM Bifrost @ DPR 2. The baseline is trustworthy**: `field` read **20.43** and **20.03** ninety seconds apart — a 0.4 ms spread, against the day's earlier 26.8–59.9 on a hot device. CSS box was 360x570 on the field windows and 360x609 on the board ones (the chrome collapses), which moves the overlay's pixel count though not the pinned scene buffer | **THE BOARD DRAWS ITS MEDIAN FRAME IN 18 ms (56 fps) AND ITS MEAN IN 26.3 ms (38 fps)**; empty it is 16 ms median (63) and 20.2 mean (50). The report's `fps` is derived from the MEDIAN, so quoting it alone overstates the window — both are given · **the decomposition: empty map 20.2 ms · symbology +5.2 · the whole 150-car fleet +0.9** · **the fleet subtraction holds despite the two windows differing in symbol count (82 against 190)**, because the CPU split shows the symbology cost is nearly identical between them — `sym:units` 4.04 against 4.16, `sym:labels` 1.50 against 1.64 · **and that is the finding underneath**: hiding 110 of 150 marks moved `sym:units` by ~0.1 ms of 4.1, so the per-unit cost is NOT the drawing — every unit is still projected and still records a hit area, because picking runs off it. The drawing-side levers are spent; the per-unit work is what is left |
| **09-05** | **[mobile-per-unit-allocation](opensa-engine/2026-09-05-mobile-per-unit-allocation.json)** | **The per-unit path stops allocating** — three objects per entity per frame removed: `gtaToEngine`'s coordinate array, `project`'s returned point and the hit rect. `projectInto(x, y, z, out)` takes three numbers and writes into a point the layer owns; the hit rects are pooled and reused; the `[...chipAreas, ...areas]` copy went with them. Taken against [the decomposition](opensa-engine/2026-09-05-mobile-board-decomposition.json) 40 minutes earlier, app **`aa64e84+`**, same pak, pin and pose | **MGA-LX3 / ARM Bifrost @ DPR 2**, 150 units + ~40 calls, fleet loaded (3 571 draws) and 80–82 marks drawn in both, css 360x609 in both. **The empty map is the control** and it is untouched by any of this: **20.03–20.43 before, 20.25 after**, so the device had not moved | **`sym:units` 4.04 → 1.62 ms (−60 %) at the same mark count** — the allocation WAS the cost, exactly as the decomposition predicted and as three drawing-side levers had each failed to show · **CPU body 11.04 → 7.86** (more than `sym:units` alone, because calls paid the same allocations and the concatenation went too) · **frame 26.30 → 23.81 ms**, so the board's cost OVER the empty map falls **+6.1 → +3.6 ms** · **p50 unchanged at 18, p95 50 → 38** — the shape of a tail losing collector pauses rather than a floor moving; half the frames were already on one display interval and still are · also fixed here: the **boot flash** the operator caught, where the mark rule asked whether a car was drawn RIGHT NOW and so opened the console with 150 chevrons before dropping 110 (`marksHidden` 109 while `unitsAsModels` was still 0, confirmed on the device) |
| **09-05** | **[mobile-nobloom-refly](opensa-engine/2026-09-05-mobile-nobloom-refly.json)** | **THE 7.7 ms IS DEAD.** The re-flight 201/9's handoff demanded before any post-chain design: the sweep priced the bloom chain at 7.7 ms of a 23.4 ms frame on an app that predates the half-resolution prefilter shipping as the console default, so it priced a chain nothing has run since. On one baseline flown twice in the same session (19.57 / 18.88, spread 0.69) the WHOLE chain is **2.43 ms of 19.23** — under the instrument's own ~2.5 ms floor on the mean. What carries it is the ladder and the tail, which move together by whole rungs: rung-1 occupancy 78/81 % against **95 %**, p95 36/34 against **26**, and the arm is bracketed rather than trailing (19.57 → 16.80 → 18.88), which is what rules out the warm-up story that has produced three false results in this chain. `surface.ablated` read back on every arm; draws 112 and triangles 277 970 identical in all three windows. **AND WHAT THE NUMBER MEANS WAS CORRECTED THE SAME DAY**: the rung MIX alone predicts every arm's mean to within the same −0.9 ms three times over, so `dtMean` here is the MISS RATE in milliseconds and not a measure of work — there is no pool of unattributed frame time to go hunting in. Restated: the bloom chain **pushes about one frame in five past the display deadline**, which under a vsync lock is what 2.43 ms of mean actually is. CPU work stays finely measurable (`bodyMeanMs` 3.54/3.40/3.11 against 2.97); GPU work is not, since `outside` is GPU time and present wait inseparably — so a GPU change under one rung is invisible **by construction, not by noise**. |
| **09-05** | **[mobile-frame-pacing](opensa-engine/2026-09-05-mobile-frame-pacing.json)** | **THE FIRST SMOOTHNESS ROW IN THIS REPOSITORY.** Smoothness became a budget the same day (the user's call) and nothing measured it, because a rate cannot — a flat 30 fps is smooth and a 60/30 alternation is not, and they report the same mean — and neither can the histogram, since a distribution has no order in it. `frame-pacing.ts` counts TRANSITIONS as frames arrive; app `f91a688` reports them. Flown on the arm this chain knows best: **the empty map stutters on 44.6 % of consecutive frame pairs and `nobloom` on 7.9 % — 5.6x fewer**, with p95 34 against 18 and the live readout at 60 against 49. **The same arm's MEAN difference is ~2.4-3.2 ms, at or under the instrument's floor, which no row is allowed to claim.** So the two criteria rank one change by an order of importance apart, and the smoothness one is what the operator sees. Caveat kept in the file: Android froze the tab during the field arm (one 14.9 s freeze) which destroyed `paceWorstRatio` for it while moving `stutterRate` by 2 counts in 1 456; that field is not quoted. One pairing, not a bracket. |
| **09-06** | **[desk-censuses](opensa-engine/2026-09-06-desk-censuses.json)** | **TWO INSTRUMENTS THE REPO ALREADY HAD AND HAD NEVER RUN.** Both were registered in `docs/debug/README.md` and cited nowhere else; the user asked whether anything was going unused, and these were it. **Residency**: the ring policy asks for **276 cells at every pose** where the view policy asks for 16–202 — at block zoom, where a dispatcher works, **26 against 276**, roughly ten times. **Overlay**: the label cap is NOT what limits labels on either surface — **overlap is**. 190 marks offered, 110 placed on a desk and **75 on a phone**, both well under their ceilings, so **six names in ten never reach a phone** and raising the cap changes nothing. The radar's dirty check works (914 calls repainting, **0** skipping), which is chain 4's own worry answered. **And `overlay-census.ts` was BROKEN** — it threw on `projectInto`, added in 9-08, because nothing had run it since; a registered tool with no caller rots silently and is a repair job the day it is needed. Neither row is a timing: both drive the shipped code over fakes and count what it ASKS FOR. |
| **09-06** | **[phone-game-data-tools](opensa-engine/2026-09-06-phone-game-data-tools.json)** | **THE FIRST RUNS OF THE GAME-DATA TOOLS ON THE MACHINE THAT HAS GAME DATA** — 62 of the 100 scripts in `scripts/debug/` read game data, the phone is the only machine holding any, and none had ever been run until `phone_run debug` reached them. **COUNTS, NOT TIMINGS**: nothing here is comparable with a frame row. | **MGA-LX3 / Termux, node v24.18.0**, `game-src/original` on the phone — **and that tree is NOT verified to be the copy the earlier doc baselines were taken on**, which is the reading every disagreement below turns on. | **Thirteen tools run, four fixed.** `dat-order-check` **9 406 rows, 0 late** (the doc said 9 268 — same verdict, different copy); `lod-link-check` **6 108 links, 0 out of range**; `txd-dimension-census` **2 760 dictionaries, 26 126 textures, ZERO unaligned DXT**; `lamp-census` 26 models with no standard lamp material. **The class this row exists for**: a tool whose default names a tree this machine does not have, read UNGUARDED, dies with a stack trace instead of a sentence — four were broken that way, and the grep that found them was wrong twice in three |
| **09-06** | **[optimisation-levers](opensa-engine/2026-09-06-optimisation-levers.json)** | **TWO LEVERS PRICED WITHOUT FLYING ANYTHING**, because this device can no longer answer a timing question below its own floor: under the vsync lock `dtMean` is a miss rate and a GPU lever under ~2.5 ms is invisible by construction. Both figures count WORK. | Desk (`residency-census`, deterministic — re-run identical to the 09-06 desk row) and `phone_run debug texture-budget` over `build/phone-cars/pak` (07:40 05-09-2026, MODELS=1, ASTC, **8 cells**). | **Residency**: the ring policy asks **276 cells at every pose**, the view policy **16–202** — at block · plan **16 against 276 (~17x)**, which is where a dispatcher works. **THE CATCH: it is not strictly cheaper** — it asks for MORE HD near the camera (16 against 12). **Textures**: 619 layers, 18.8 M texels, **25.4 MB resident against 15.7 MB on disk**, and the model lands within **0.4 MB** of the 08-12 field capture, so this can be decided without flying. **On this district textures are NOT the constraint** |
| **09-09** | **[mobile-box-pin-verification](opensa-engine/2026-09-09-mobile-box-pin-verification.json)** | **`?box=WxH` FLOWN — AND ITS OWN PREMISE REFUTED.** The pin shipped 09-08 as a *verification* arm on the reasoning that *360x320 is the box every existing row was taken at*. The filed record says otherwise before any flight: **eleven distinct CSS boxes across the 28 rows that state one** (360 wide at 320/364/373/377/432/444/491/550/570/609, plus 403x334), single rows spanning up to six. This device's natural box is now **360x654** — a twelfth. | **MGA-LX3 / ARM Bifrost @ DPR 2**, pak `19:23 28-08-2026` ASTC over `los-santos-centre`, app `ea5dc22`, board EMPTY, one settled window an arm back to back on the charger (`boxed` first, `field` 34 s later), `hiddenMs 0` and `warnings: []` in both. **`?surface=720x640` pinned the scene buffer in BOTH**, so draws (112), triangles (277 970) and residency (66.61 MB) are identical — the pair varies the CSS box and therefore the overlay canvas and the COMPOSITOR's scale (1:1 against a 2.04x upscale), nothing the GPU is asked for. | **The pin works**: `device.css` read back 360x320, `boxPinned: true`, min == max on both axes. **No box cost in the CPU body**: `overlay:clear` 0.131/0.117 is a ratio of **1.114 on twice the pixels** — indistinguishable from `engine-frame`'s **1.116**, which cannot depend on the box at all — and `overlay-2d` is flat (0.996) because an empty board draws nothing. **The miss rate moved and MAY NOT BE CLAIMED**: 24.1 % against 15.7 % past 16 ms (dtMean 17.83/17.34, under the 2.47 ms null-arm floor), one unbracketed window an arm flown in drift's own direction. **And `visibility` earned its place**: the session's first window — 195 s, 10 136 frames, every number plausible — was discarded on `hiddenMs 7732.5` alone |
| **09-09** | **[phone-audio-census](opensa-engine/2026-09-09-phone-audio-census.json)** | **THE AUDIO CHAIN'S GATE (203/1-01), PASSED — AND IT CAUGHT A 720-BYTE ERROR GOING THROUGH.** Every number in the audio concept's format table came from documentation rather than a file. `BANK_HEADER_BYTES` was **4 084** and the arithmetic never closed (4 + 400 × 12 = 4 804); the census does not argue with documents — it differences consecutive bank offsets against the PCM size `BankLkup` states, and **361 gaps all read 4 804**. Shipped, every sound would have played 720 bytes early, from inside the tail of the sound before it, and silently: the shift is constant, so lengths stay positive and nothing overruns. One bank declaring 380 sounds is what gave it away. | **COUNTS, NOT TIMINGS** — nothing here compares with a frame row. MGA-LX3 / Termux, the phone's stock `game-src/original` with `audio/{CONFIG,SFX,streams}`, run through the panel (`phone_run debug SCRIPT=audio-census`). | **9 packages · 370 banks · 8 857 sounds · 351 looping · 364.1 MB of PCM**, per bank 1 / 5 / 380. **`LAYOUT AGREES`** on the re-run. **103 distinct sample rates** — 12 000 Hz carries 59 % of every sound, 15 000 another 22 %, 8 000 a further 11 %, while 22 050 + 44 100 + 11 025 together are **1.5 %**; one sound is authored at 2 021 Hz. **364.1 MB against the chain's 64 MB budget is 5.7x**, so range fetching is settled by measurement rather than argument — but SCRIPT alone is **304.9 MB (83.7 %)** and FEET + GENRL + PAIN_A come to **28.9 MB**. **155 audio zones, all in one file** (`audiozon.ipl`, 152 box + 3 sphere, 151 active) — 4/01 is a single load, not a scan |
| **09-11** | **[phone-panel-audio-first-flight](opensa-engine/2026-09-11-phone-panel-audio-first-flight.json)** | **204/5-01's FIRST FLIGHT, AND IT IS NOT THE ROW — it found a SILENT ABSENCE instead.** 97.5 s on a 150-unit / 40-call board with the context actually RUNNING (checked first, because a capture taken at `waiting` measures a silent console rather than a quiet one). **No car is voiced**: `audio.vehicles` 0, `units.unvoiced` 150 — this pak carries none of `gtasa_vehicleAudioSettings.cfg`, `vehicles.ide`, `handling.cfg`, and `loadAudio` turns each missing file into an empty map with NOTHING reported. A 150-unit board sounded like an empty one while every field stayed plausible. | **MGA-LX3 / ARM Bifrost @ DPR 2**, CSS 360x654, `?surface=720x640`, pak `19:23 28-08-2026` ASTC over `los-santos-centre`, app `8eb70e3`, vehicles converted `admiral,comet,infernus` (the pak refused the dispatch fleet, so no unit is drawn as a car either). **On the charger the whole run** — `power.charging` true — so no battery delta could have been read from it. One half only; no `boardsilent` partner. | **READABLE**: audio clock **mean 0.70 ms** against the 2 ms budget, **max 7.5 ms** — 3.7x over, one tick in 976 · panel **32 played, 96 coalesced**, 3 of every 4 raised events a repeat inside the 900 ms per-name window on a busy board · frame **63 fps, p50 16 / p95 34** beside it · `cad.online` true, 1 message delivered, `assumed` 0. **NOT MEASURED AND SAY SO**: `refusedByBus.cad` = 0 is true and MEANINGLESS (2 live voices out of 64 — the reserve was never asked for) · `peakSample` 0.7688 = **−2.29 dBFS** of a single panel tone, NOT of 64 summed voices, so 1/02's −4.8 dBFS arithmetic is untouched · `maxLatencyMs` 0 **by construction, not by speed** — no event in this build carries an arrival timestamp, so the 50 ms budget has no instrument yet · MB against 64 is 0 because no car sound was ever fetched |
