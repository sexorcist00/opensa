...
| **09-05** | **[mobile-map-ablation-sweep](opensa-engine/2026-09-05-mobile-map-ablation-sweep.json)** | 201/9's **ablation sweep, four arms of seven** — the instrument that replaces `timestamp-query`: a pass is priced by REMOVING it and re-flying the SAME ten-leg route, each window the delta of two histogram readings. `field` / `nobloom` / `bloom4` / `nocloud`, pak `19:23 28-08-2026` (astc, `los-santos-centre`), app **`5937214+`**, `?surface=720x640` on every arm, `surface.ablated` read back before any number was kept | **MGA-LX3 / ARM Bifrost @ DPR 2**, board EMPTY, same rect and same route as the 09-04 ladder. **THE BASELINE IS ALSO A CONTROL**: the previous session's link could hand the camera a pose with no heading, which gives a NaN eye and draws BLACK — and a black frame is a CHEAP frame, so nothing in a capture would have said so. `field` re-flown on the fixed app reads **23.4 ms** against the ladder's **24.5**, p50 20 vs 20, 29 cell creates vs 29 — **the 09-04 ladder stands** | **The bloom chain is 7.7 ms of a 23.4 ms frame** and removing it is the only change yet measured on this device that reaches the declared 60: `nobloom` **15.8 ms** mean, **607 of 614 frames on one display interval**, p95 18 against 38. **And its TAIL is free**: `bloom4` (8 levels → 4, every level 9/05 named as waste — 12x10, 6x5, 3x3 px here) reads **23.2 ms**, which is the baseline. So the cost is the FIRST levels, at full and half resolution — the post chain's BANDWIDTH, as 09-04's `rgb10a2` arm already implied — and **9/05 as written would buy nothing; it needs re-aiming, not building** · `nocloud` **21.6 ms** (−1.8) is 9/06's lever and the one arm here that converts straight into a fix, since the field is visually identical for minutes · `nocells`, `noprobe`, `noskylut` NOT taken: switching arms opened a second console tab twice and two consoles share one GPU — discarded rather than filed |
