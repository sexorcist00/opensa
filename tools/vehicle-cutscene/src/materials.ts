/**
 * Carcols paint bake (plan 002 step 5). The cutscene system never applies carcols, so the Kam's/ZModeler
 * paint MARKER colours render raw (the gate-4 green/pink). Vanilla cs models ship their paint baked into
 * the material colours; this pass does the same: every paint-marker material gets the model's first
 * carcols combo, alpha preserved. LAMP markers (`vehiclelights*` tints) are left alone — vanilla cs
 * models keep those too.
 *
...
function bakeGeometry(geometry: RwChunk, colours: null | PaintColours): number {
  ...
  for (const material of readRw(materialList.data).chunks) {
    ...
    const slot = PAINT_MARKERS.get(`${struct.data[4]},${struct.data[5]},${struct.data[6]}`);
    if (slot === undefined) {
      continue;
    }
    if (!colours) {
